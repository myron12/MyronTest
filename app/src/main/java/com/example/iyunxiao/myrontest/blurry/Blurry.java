package com.example.iyunxiao.myrontest.blurry;
...
import com.example.iyunxiao.myrontest.blurry.internal.BlurTask;
import com.example.iyunxiao.myrontest.blurry.internal.Helper;
...
//    }
        ...
        public ImageComposer capture(Bitmap capture) {
        ...
        public void onto(final ViewGroup target) {
            ...
            if (async) {
                ...
                Bitmap capture = target.getDrawingCache();
                BlurTask task = new BlurTask(context, capture, factor, new BlurTask.Callback() {
                    @Override
...
//      this.listener = listener;
//    }
        public ImageComposer(Context context, Bitmap capture, BlurFactor factor, boolean async,
                             ImageComposerListener listener) {
            this.context = context;
            this.capture = capture;
            this.factor = factor;
        ...
        public void into(final ImageView target) {
            factor.width = capture.getWidth();
            factor.height = capture.getHeight();
            ...
            if (async) {
                ...
                Drawable drawable = new BitmapDrawable(context.getResources(), Blur.of(context, capture, factor));
                target.setImageDrawable(drawable);
            }
        }
